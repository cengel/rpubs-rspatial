...
Before we start, please make sure you have the `dplyr`, `sp`, `rgdal`, and `rgeos` installed. For `rgdal` installation inststructions refer to [this earlier notebook](http://rpubs.com/cengel248/56015). 


__Mac only__: There currently is no binary version of the latest `rgdal`. If installation of `rgeos` on Mac does not work, try to install this earlier version, which *is* provided as binary:

```{r eval=FALSE}
require(devtools)
install_version("rgeos", version = "0.3-11", repos = "http://cran.us.r-project.org")
```

Or see [here](http://dyerlab.bio.vcu.edu/2015/03/31/install-rgeos-on-osx/) to compile your own from source.
...
To do this we will use `left_join` from the [`dplyr`](http://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html) package [mentioned earlier](http://rstudio-pubs-static.s3.amazonaws.com/54694_f7eecf57c1a443db9356d3d1aab9c4c5.html#more..)[^4]. 

[^4]: The `geo_join()` command from the recently released [`tigris` package](https://cran.r-project.org/web/packages/tigris/index.html) also provides a convenient way to merge a data frame to a spatial data frame.
